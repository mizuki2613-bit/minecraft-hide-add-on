...
# Restore mainhand
$execute unless items entity $(target) mob.inventory.5 $(ignore) run summon item ~ ~ ~ {Tags:[blockhide.drop_mainhand],Item:{id:stone}}
data modify entity @n[tag=blockhide.drop_mainhand,distance=0] Owner set from entity @s UUID
$data modify entity @n[tag=blockhide.drop_mainhand,distance=0] Item set from entity $(target) Inventory[5]
